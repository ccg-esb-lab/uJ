...
print("Loading: "+list_pos.length+" positions/"+list_frames.length+" frames/"+str_channel.length+" channels");
...
for(pos=0;pos<list_pos.length;pos++){	
	...
	print("Loading "+list_rois.length+" Images from "+dirROIS+" channel "+currentChannel);
	...
	for(i=1; i<list_rois.length; i++){
		...
		thisChannel=currentChannel;  //TMP
