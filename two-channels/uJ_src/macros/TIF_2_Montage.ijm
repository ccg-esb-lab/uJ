...
//for(i=0; i<2; i++){
	...
	fileNameGFP=expeLabel+"_"+thisPos+"_GFP_001.tif";
	run("Image Sequence...", "open="+dirTIF+thisPos+"/GFP/"+fileNameGFP+" sort");
	fileNameDsRed=expeLabel+"_"+thisPos+"_DsRed_001.tif";
	run("Image Sequence...", "open="+dirTIF+thisPos+"/DsRed/"+fileNameDsRed+" sort");
	
	
	run("Merge Channels...", "c1=DsRed c2=GFP keep");
	//run("Merge Channels...", "c1=GFP c2=DsRed keep");
	...
	//for(s=1;s<=nS;s+=time_sample_frames){
	for(j=0;j<subset.length;j++){
		...
		fillRect(0, 0, 640, 30);
		//rename("Time: "+s+"-"+red+"|"+green+"|"+blue);
		frame_name=expeLabel+"_"+thisPos+"_Frame_"+s;
		rename(frame_name);
	...
	run("Images to Stack", "name=StackRGB title=[Frame] use");
	if(flag==0){
		selectWindow("StackRGB");
		run("Duplicate...", "duplicate");
		selectWindow("StackRGB-1");
		run("Canvas Size...", "width=640 height=30 position=Top-Center");
		run("Make Montage...", "columns="+nSlices+" rows=1 scale=1 ");
		rename("xy00");
		saveAs("Tiff", dirMontage+expeLabel+"_Montage_"+"xy00"+".tif");
		flag=1;
		selectWindow("StackRGB-1");
		close();
		}
	...
	saveAs("Tiff", dirMontage+expeLabel+"_Montage_ALL"+".tif");
	selectWindow(expeLabel+"_Montage_ALL"+".tif");
	close();
