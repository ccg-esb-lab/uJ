...
/********************************************************************/
	...
	if(pMode!="All"){
		...
		labels_index = newArray("natural","by layers","reverse");
		...
		if(reindex=="Reverse"){
			list_frames_reindexed=Array.reverse(list_frames_reindexed);	
			}
...
lutPath=dirROOT+"uJ_src_pHT/luts/glasbey.lut";
...
function detectCell(x, y){
	if( getPixel(x,y) > threshold_color ) { //Only if not black
